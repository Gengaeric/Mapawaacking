...
export default async function AdminPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { q: query, province, crew, eventType, startYear, startYearFrom, startYearTo, includeDeleted } = parsedFilters;
  ...
  const [peopleResult, eventsResult, profilesResult] = await Promise.allSettled([
  ...
  if (peopleResult.status === "rejected") dataLoadErrors.push(`people: ${peopleResult.reason instanceof Error ? peopleResult.reason.message : "error desconocido"}`);

  const events = eventsResult.status === "fulfilled" ? eventsResult.value : [];
  if (eventsResult.status === "rejected") dataLoadErrors.push(`events: ${eventsResult.reason instanceof Error ? eventsResult.reason.message : "error desconocido"}`);

  const profiles = profilesResult.status === "fulfilled" ? profilesResult.value : [];
  const missingProfilesTable = profilesResult.status === "rejected" && isMissingProfilesTableError(profilesResult.reason);
  if (profilesResult.status === "rejected" && !missingProfilesTable) {
    dataLoadErrors.push(`profiles: ${profilesResult.reason instanceof Error ? profilesResult.reason.message : "error desconocido"}`);
  }

  const profileById = new Map(profiles.map((p) => [p.user_id, p.email]));

  const [auditRowsResult, totalPeopleResult, totalEventsResult, totalEditionsResult, totalParticipationResult, peopleByProvinceResult, peopleByYearResult, eventsByTypeResult, eventsByProvinceResult] = await Promise.allSettled([
    listAuditLogs(),
  ...
  const auditRows = auditRowsResult.status === "fulfilled" ? auditRowsResult.value : [];
  if (auditRowsResult.status === "rejected") dataLoadErrors.push(`audit: ${auditRowsResult.reason instanceof Error ? auditRowsResult.reason.message : "error desconocido"}`);

  const totalPeople = totalPeopleResult.status === "fulfilled" ? totalPeopleResult.value : [];
  const totalEvents = totalEventsResult.status === "fulfilled" ? totalEventsResult.value : [];
  const totalEditions = totalEditionsResult.status === "fulfilled" ? totalEditionsResult.value : [];
  const totalParticipation = totalParticipationResult.status === "fulfilled" ? totalParticipationResult.value : [];
  const peopleByProvince = peopleByProvinceResult.status === "fulfilled" ? peopleByProvinceResult.value : [];
  const peopleByYear = peopleByYearResult.status === "fulfilled" ? peopleByYearResult.value : [];
  const eventsByType = eventsByTypeResult.status === "fulfilled" ? eventsByTypeResult.value : [];
  const eventsByProvince = eventsByProvinceResult.status === "fulfilled" ? eventsByProvinceResult.value : [];

  const builderEqConfirmed = [...peopleQueryDebugCalls, ...eventsQueryDebugCalls].every((line) => line.includes('.eq("'));
    ...
    <main className="auth-layout">
      ...
      {debugMode ? (
        <section data-debug-filters style={{ background: "#f5f5f5", borderRadius: 8, padding: 12, marginBottom: 12 }}>
          ...
          <p>builderEqConfirmed: {String(builderEqConfirmed)}</p>
      ...
      {debugMode && dataLoadErrors.length ? (
        <section style={{ border: "1px solid #f59e0b", borderRadius: 8, padding: 12, background: "#fffbeb", marginBottom: 12 }}>
          <p>admin load warnings:</p>
          <ul>{dataLoadErrors.map((warning) => <li key={warning}>{warning}</li>)}</ul>
        </section>
      ) : null}
